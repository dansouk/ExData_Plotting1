...
# Size estimate
# One date, one time, seven decimal columns * 2,075,259 rows
rows = 2075259
rowLength = (object.size(Sys.Date) + object.size(Sys.time) + (object.size(999.999) * 7))
estSize = rowLength * rows
# 11023775808 bytes or about 11 Gb
# This will not be a problem, so I'll just load the entire file, then subset it for simplicity
# If memory was a constraint, it would be better to come up with an approach to read the file
# in chunks and apply a filter to each chunk to create the subset

# Load data
dataPath = 'C:/Users/dansouk/Source/Repos/coursera-datascience-johnshopkins/04-ExploratoryDataAnalysis/01-Graphs/course_project/'
setwd(dataPath)
dataFile = 'household_power_consumption.txt'
dataPathFile = paste(dataPath, dataFile, sep='')
data = read.csv(dataPathFile, sep = ';', na.strings = '?')
# Fix up the Date column. It comes in as dd/mm/yyyy
data$Date = strptime(data$Date, "%d/%m/%Y")

# Get only the required two days, Feb 1 and 2, 2007
...
# Drop the full dataset to free up some memory
rm(data)
...
# Save the data so that our plot scripts can load it
save(dataFeb, file='dataFeb.RData')
